var SVG = d3.select("body").append("svg").attr({"height":window.innerHeight-20,"width": window.innerWidth-20}).style({'position':'fixed','top':'0px','left':'0px','z-index':'-1'});
var globalDepth = 40;
var radius = 60;
var thickness = 10;
var rootX = window.innerWidth;
var rootY = window.innerHeight;
var globalMaxBranchNo = 4;
var treeStruct = {
    x1: rootX,
    y1: rootY,
    x2: rootX,
...
var randomno = 0;
var flag = 1;
function addChild(node, depth, i, maxBranchNo){
	...
	for(var j=0;j<maxBranchNo;j++){
	    addChild(node.children[j], depth, i, maxBranchNo);
	}
    }
}
...
for(var i =0; i<treeNo; i++){
    ...
    depth: i
        ...
        var branchNo = Math.ceil(Math.random() * globalMaxBranchNo)+1;
    for(var ctr =0; ctr<15;ctr++){
        addChild(treeStruct, 1, i, branchNo);
    }
